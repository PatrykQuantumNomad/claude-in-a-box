...
must_haves:
  truths:
    ...
    - "Liveness probe exits 0 when Claude Code process is running and exits 1 when it is not"
    - "Readiness probe exits 0 when Claude Code is authenticated and exits 1 when it is not"
    - "Container image ships probe scripts at expected /usr/local/bin/ paths and wires liveness into HEALTHCHECK"
  artifacts:
